...
class SairController extends Zend_Controller_Action
{
	...
	public function init()
	{
		...
		$this->view->attr = (Object) array( "title" => $this->config->project->planc->sair." Install's Graph" );
					
		// Check Session
		$this->getFrontController()->getParam("bootstrap")->checkAuthentication($this->config->project->id->sair);
	...
	public function installscsvAction(){
		...
		$path = "uploads/SAir_installs_".date("YmdHis").$this->config->csv->ext;
	...
	public function reviewcsvAction(){
		...
		$path = "uploads/SAir_review_".date("YmdHis").$this->config->csv->ext;
		$this->library->exporter->exportToCSVFromDB($result, $path);
		echo $path;
	}
	...
	{
		$this->view->attr->title = "S Air Carrier's Graph";
	}
	...
	public function carriercsvAction(){
		...
		$path = "uploads/SAir_carrier_".date("YmdHis").$this->config->csv->ext;
		$this->library->exporter->exportToCSVFromDB($result, $path);
		echo $path;
	}
	...
	{
		$this->view->attr->title = "S Air Device's Graph";
	}
	...
	public function devicescsvAction(){
		...
		$path = "uploads/SAir_devices_".date("YmdHis").$this->config->csv->ext;
		$this->library->exporter->exportToCSVFromDB($result, $path);
		echo $path;
	}
	...
	{
		$this->view->attr->title = "S Air Error's Graph";
	}
	...
	public function errorcsvAction(){
		...
		$path = "uploads/SAir_error_".date("YmdHis").$this->config->csv->ext;
		$this->library->exporter->exportToCSVFromDB($result, $path);
		echo $path;
	}
}