...
app.controller('searchCrtl', function ($scope, $http, $sce, $filter) {
	...
	$scope.loadTotal = function()
	{
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/merchant/gettotalredeem",
								data: $.param({"q" : q, "promo_id" : promo_id, "promo_name" : promo_name, "merchant_name" : merchant_name, "voucher_id" : voucher_id, "channel":channel, "type":type, "start_claim" : start_claim, "end_claim" : end_claim, "start_redeem" : start_redeem, "end_redeem" : end_redeem}),
	...
	{		
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/merchant/loadmore",
								data: $.param({"page" : page, "q" : q, "promo_id" : promo_id, "promo_name" : promo_name, "merchant_name" : merchant_name, "voucher_id" : voucher_id, "channel":channel, "type":type, "start_claim" : start_claim, "end_claim" : end_claim, "start_redeem" : start_redeem, "end_redeem" : end_redeem}),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
							});
		responsePromise.success(function(data, status, headers, config) {
	...
	$scope.exportCurrent = function($event)
	{
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/merchant/exportcsv",
	...
	$scope.search_promo = function(promo_id,batch_id,campaign_id){
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/cssgift/searchpromo",
								data: $.param({"promo_id" : promo_id, "batch_id" : batch_id ,"campaign_id" : campaign_id }),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
							});
		...
		console.log('not sure calvin ' + promo_id + ' ' + batch_id+ ' ' + campaign_id);
	}
	...
	$scope.exportAll = function($event)
	{
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/merchant/exportcsv",
								data: $.param({"q" : q, "promo_id" : promo_id, "promo_name" : promo_name, "merchant" : merchant_name, "voucher_id" : voucher_id, "channel":channel, "type":type, "start_claim" : start_claim, "end_claim" : end_claim, "start_redeem" : start_redeem, "end_redeem" : end_redeem}),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
							});
		$(".bg-log-loading").show();
		responsePromise.success(function(data, status, headers, config)
	...
	$scope.exportClaim = function($event, type)
	{
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/merchant/exportclaim",
	...
	$scope.createPanelPromo = function(data){
		...
		"<td> " +data.remaining_redeem+"</td>"+
		"<td> " +data.total_claim+"</td>"+
		"<td>" +data.remaining_claim+"</td>"
		);
	};
	...
	return {
		link: function($scope, elem, attrs){
			elem.bind("click", function() {
				...
				$scope.search_promo(attrs.promodata, attrs.batchdata, attrs.campaigndata);
				$(".cover").show();
...
$(document).ready(function(){
	...
	$('#redeem').change(function() {
		if(selects.options[selects.selectedIndex].value == "redeem_time") {
			//alert("redeem");
		...
		} else if(selects.options[selects.selectedIndex].value == "claim_time") {
			//alert("claim");
