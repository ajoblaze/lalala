...
$(function () {
	...
	if($("body").length > 0) {
		...
		//initial call
		$.ajax({
			url: base_url+"/managesamsung/pullsfit",
			type: "GET",
		...
		function tableDraw() {
			...
			for(i = total_data-1; i >= 0; i--) {
				$("table#sfit_daily_install tbody").append("<tr><td align='center'>" + categories_data[i] + "<td align='center'>" + values_data_in[i] + "</td>" + "<td align='center'>" + "<a href="+base_url+"/managesamsung/edit?id=" + id_data[i] + "><button type='button' class='btn btn-primary'><span class='glyphicon glyphicon-pencil'></span></button></a>"  + "&nbsp;&nbsp;" + "<button type='button' data-date='"+ date_data[i] +"' data-id='"+ id_data[i] +"' class='btn btn-danger btn-delete btdeletesfit'><span class='glyphicon glyphicon-trash'></span></button>" + "</td></tr>");
			}
			
			$(".btdeletesfit").click(function(e)
			{
				var tgl = $(this).data("date");
				var id = $(this).data("id");
				var conf = confirm("Apa anda yakin ingin menghapus data install sfit pada tanggal " + tgl + " ?");
				if (conf == true) {
					window.location = base_url+"/managesamsung/delete?id=" + id + "&value=sfit";
				}
			});
		}
