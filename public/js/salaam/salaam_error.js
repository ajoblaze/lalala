...
$(function () {
	var data; var data1; var data2; var max_value; var min_value; var values_data_in; var values_data_un;
	...
	$("#apply_date").click(function() {
		...
		$.ajax({
			url: base_url+"/salaam/errorreportin",
			type: "POST",
	...
	if($("body").length > 0) {
		...
		function tableDraw() {
			...
			$("table#salaam_daily_install tbody").empty();
			$("table#salaam_install tbody tr").html("<td align='center' data-toggle='tooltip' title='The number of application is not responding and the number of error occurance'><h1><b>" + formatNumber(total_active_in) + "</b></h1></td>" + "<td align='center' data-toggle='tooltip' title='The number of application is not responding'><h1><b>" + formatNumber(total_un) + "</b></h1></td>" + "<td align='center' data-toggle='tooltip' title='The number of error occurance'><h1><b>" + formatNumber(total_in) + "</b></h1></td>");
			for(i = total_data-1; i >= 0; i--) {
				
				$("table#salaam_daily_install tbody").append(
				"<tr><td align='center'>" + categories_data[i] + "</td>"+ "<td align='center'>" + formatNumber(values_data_in[i]) + "</td><td align='center'>" + formatNumber(values_data_un[i]) +"</td>" + "<td align='center'>" + 
				
				"<a><button type='button' data-flag='1' data-tgl='"+categories_data[i]+"' class='btos btn btn-danger' ><span></span>&nbsp; <span class='action-text'>"+os[i]+", Details</span>&nbsp; </button></a>" + "</td>" + "<td align='center'>" + 
				"<a><button type='button' data-flag='2' data-tgl='"+categories_data[i]+"' class='btdevice btn btn-warning'><span></span>&nbsp; <span class='action-text'>"+device[i]+", Details</span>&nbsp; </button></a>" + "</td>" + "<td align='center'>" + 
				"<a><button type='button' data-flag='3' data-tgl='"+categories_data[i]+"' class='btappversion btn btn-primary'><span></span>&nbsp; <span class='action-text'>"+app_version[i]+", Details</span>&nbsp; </button></a>" + "</td>"+ "<td align='center'>" + 
				"<a><button type='button' data-flag='4' data-tgl='"+categories_data[i]+"' class='btdetail btn btn-info'><span></span>&nbsp; <span class='action-text'>"+detail[i]+", Details</span>&nbsp; </button></a>" + "</td></tr>");
			}
			...
			$(".btos").click(function(e)
			{
				...
				$.ajax({
					url: base_url+"/salaam/errorpopupin",
					type: "POST",
					dataType: "html",
			...
			function popup_error(field) {
			 ...
			 $('#error_column').html(field)
				...
				{
					$('#popup_error').append(
					"<tr><td>" +data1[x]['datelabel']+"</td>"+
					"<td><div style='width:480px;word-break:break-all;'>" +data1[x][field]+"</div></td>"+
