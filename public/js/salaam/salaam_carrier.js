...
$(function () {
	...
	$("#export").click(function(){
		$.ajax({
					url: base_url+"/salaam/carriercsv",
					type: "POST",
	...
	$("#apply_date").click(function(){
		...
		start_date = $.datepicker.formatDate( "yy-mm-dd", $("#datepicker1").datepicker("getDate"));
				$.ajax({
							url: base_url+"/salaam/pullcarrier",
							type: "POST",
	...
	if($("body").length>0){
		//initial call
			...
			function tableDraw(){
				...
				$("table#slime_install tbody tr").html("<td style='text-align:center;'><h1><b>"+ formatNumber(total_install) +"</b></h1></td>"+"<td style='text-align:center;'><h1><b>"+ formatNumber(res[2]) +"</b></h1></td>");
				for(var i = 0;i<total_data;i++){
				//	console.log(data_list[i][0]);
					$("table#slime_daily_install tbody").append("<tr><td>"+ all_data_list[i][0]+"</td>"+"<td>"+ formatNumber(all_data_list[i][1]) +"</td></tr>" );
				}
			}
			var init = function(){
