...
var pid = $("#hid_param_pid").val();
var pname = $("#hid_param_pname").val();
...
payment.controller("paymentCtrl",function($scope,$filter,$http)
{
    ...
    $scope.getDetail = function ($event,data)
    {
		...
		$scope.callback = $.parseJSON(data.callback_handler_content);
        var detail=$http({
                        method : 'POST',
                        url :base_url+"/csslime/detailtransaction",
	...
	{		
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/csslime/loadmore",
								data: $.param({"page" : page, "search" : search, "pid" : pid, "pname" : pname,"order_id" : order_id, "name" : name, "email" : email, "start_order" : start_order, "end_order": end_order, "transaction_id":transaction_id, "payment_type" : payment_type, "status" : status}),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
							});
	...
	$scope.exportCurrent = function($event)
	{
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/csslime/exportcsv",
