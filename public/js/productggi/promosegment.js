...
app.controller('searchCrtl',['$scope', '$http', '$sce', '$filter', function ($scope, $http, $sce, $filter) {
	...
	{		
		...
		if ($scope.isLoading == false )
		{
			...
			var responsePromise = $http({
									method: 'POST',
									url: base_url+"/productggi/loadmorepromosegment",
									data: $.param({"page" : page, "q" : q, "title": title, "start_date": start_date, "end_date": end_date, "period":period}),
	...
	$scope.loadTotal = function()
	{
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/productggi/gettotalpromosegment",
								data: $.param({"q" : q, "title": title, "start_date": start_date, "end_date": end_date, "period":period}),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
								});
		responsePromise.success(function(data, status, headers, config) {
	...
	$scope.search_detail = function(id){
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/productggi/getpromosegmentdetail",
	...
	console.log(data.segment_rule_input);
			var rule_input = $.parseJSON(data.segment_rule_input);
			var table="";
			for(var x=0;x<rule_input.length;x++){
			
			table=table+'<tr><td>'+rule_input[x].toString()+'</td></tr>';
			}			
			$("#detail_user").html(	//gue ganti
			'<div class="form-horizontal">'+
			'<div class="promo_content">'+
				'<div class="form-group">'+
				'<label class="col-sm-5 control-label" for="inputEmail3">Title</label>'+
			'<label class="col-sm-7 control-label" for="inputEmail3">'+data.title+'</label>'+
			'</div>'+
			'<div class="form-group">'+
			...
			'</div>'+
			'<div class="form-group">'+
				'<label class="col-sm-5 control-label" for="inputEmail3">Segment Rule Type</label>'+
			'<label class="col-sm-7 control-label" for="inputEmail3">'+data.segment_rule_type+'</label>'+
			'</div>'+
			'<div class="form-group">'+
				'<label class="col-sm-5 control-label" for="inputEmail3">Segment Rule Input</label>'+
			'</div>'+
			'<div class="form-group">'+
				'<label class="col-sm-12 control-label" style="height:300px;overflow-y:scroll"> <table class="table table-striped">'+
					table
				+'</table></label>'+
			'</div>'
			);
	...
	$scope.exportAll = function($event)
	{
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/productggi/exportcsvpromosegment",
								data: $.param({"export" : 0, "q" : q, "title" : title}),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
							});
		$(".bg-log-loading").show();
		responsePromise.success(function(data, status, headers, config)
