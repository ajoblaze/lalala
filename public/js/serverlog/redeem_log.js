var hilang =false;
var pernah=false;
var app = angular.module('server_redeem_log', ['ngSanitize']);
...
app.controller('redeem_logCrtl', function ($scope, $http, $sce, $filter) {

	$scope.search_list =$.parseJSON( $('#hid_redeem_log').html() );
	$scope.loading = false;
	$scope.query = {};
	$scope.queryBy = "$";
	$scope.start_date = "";
	...
	$scope.loadTotal = function()
	{
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/serverlog/gettotalredeem",
								data: $.param({"q" : q, "email" : email, "imei_or_mac" : imei, "merchant" : merchant, "model" : model, "promo_name":promo_name, "status":status, "status_code" : status_code, "start_redeem" : start_redeem, "end_redeem" : end_redeem}),
	...
	$scope.exportCurrent = function($event)
	{
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/serverlog/exportcsvredeemlog",
	...
	$scope.exportTop1000 = function($event)
	{
		var responsePromise = $http({
								...
								data: $.param({
									...
									"start_redeem" : $("#start_redeem").val(),
									"end_redeem" : $("#end_redeem").val()
								}),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
							});
		$(".bg-log-loading").show();					
	...
	{
		var now = new Date();
		var date = new Date(data.redeem_time.replace(/-/g, "/"));
	...
	{		
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/serverlog/redeemloadmore",
								data: $.param({
									"offset" : offset,
									"q" : $("#input_search").val(),
									"email" : $("#email").val(), 
