...
var device_identifier = $("#device_identifier").val();
var segment = $("#segment").val();
var product = $("#product").val();
var start_download = $("#start_download").val();
var end_download = $("#end_download").val();
...
app.controller('searchCrtl', function ($scope, $http, $sce, $filter) {
	...
	$scope.loadTotal = function()
	{
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/download/gettotaldownload",
								data: $.param({"q" : q, "name" : name, "email" : email, "device_model" : device_model, "device_identifier" : device_identifier,
												"start_download" : start_download, "end_download": end_download, "product" : product, "segment" : segment }),
	...
	{		
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/download/loadmoredownload",
								data:  $.param({"page" : page, "q" : q, "name" : name, "email" : email, "device_model" : device_model, "device_identifier" : device_identifier,
												"start_download" : start_download, "end_download": end_download, "product" : product, "segment" : segment}),
								headers: {'Content-Type': 'application/x-www-form-urlencoded'}
							});
		responsePromise.success(function(data, status, headers, config) {
	...
	$scope.queryByDay = function (data){
		var now = new Date();
		var date = new Date(data.download_time.replace(/-/g,"/"));		
	...
	$scope.exportCurrent = function($event)
	{
		...
		var responsePromise = $http({
								method: 'POST',
								url: base_url+"/download/exportcsv",
