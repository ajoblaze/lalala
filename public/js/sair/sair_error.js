...
$(function () {
	...
	$("#apply_date").click(function() {
		...
		$.ajax({
			url: base_url+"/sair/errorreportin",
			type: "POST",
	...
	if($("body").length > 0) {
		...
		function tableDraw() {
			...
			$("table#sair_daily_install tbody").empty();
			$("table#sair_install tbody tr").html("<td align='center' data-toggle='tooltip' title='The number of application is not responding and the number of error occurance'><h1><b>" + formatNumber(total_active_in) + "</b></h1></td>" + "<td align='center' data-toggle='tooltip' title='The number of application is not responding'><h1><b>" + formatNumber(total_un) + "</b></h1></td>" + "<td align='center' data-toggle='tooltip' title='The number of error occurance'><h1><b>" + formatNumber(total_in) + "</b></h1></td>");
			for(i = total_data-1; i >= 0; i--) {
				
				$("table#sair_daily_install tbody").append(
				"<tr><td align='center'>" + categories_data[i] + "</td>"+ "<td align='center'>" + formatNumber(values_data_in[i]) + "</td><td>" + formatNumber(values_data_un[i]) +"</td>" + "<td align='center'>" + 
				
				"<a><button type='button' data-flag='1' data-tgl='"+categories_data[i]+"' class='btos btn btn-danger' ><span></span>&nbsp; <span class='action-text'>"+os[i]+"</span>, Details</button></a>" + "</td>" + "<td align='center'>" + 
				"<a><button type='button' data-flag='2' data-tgl='"+categories_data[i]+"' class='btdevice btn btn-warning'><span></span>&nbsp; <span class='action-text'>"+device[i]+"</span>, Details</button></a>" + "</td>" + "<td align='center'>" + 
				"<a><button type='button' data-flag='3' data-tgl='"+categories_data[i]+"' class='btappversion btn btn-primary'><span></span>&nbsp; <span class='action-text'>"+app_version[i]+"</span>, Details</button></a>" + "</td>"+ "<td align='center'>" + 
				"<a><button type='button' data-flag='4' data-tgl='"+categories_data[i]+"' class='btdetail btn btn-info'><span></span>&nbsp; <span class='action-text'>"+detail[i]+"</span>, Details</button></a>" + "</td></tr>");
			}
			...
			$(".btos").click(function(e)
			{
				...
				$.ajax({
					url: base_url+"/sair/errorpopupin",
					type: "POST",
					dataType: "html",
