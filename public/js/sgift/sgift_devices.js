...
$(function () {
	...
	$("#apply_date").click(function(){
		...
		start_date = $.datepicker.formatDate( "yy-mm-dd", $("#datepicker1").datepicker("getDate"));
				$.ajax({
							url: base_url+"/sgift/pulldevices",
							type: "POST",
		...
		//initial call
			$.ajax({
				...
				success: function(result){
					...
					$(".loading").hide();
					data = $.parseJSON(res[0]);
					all_data = $.parseJSON(res[1]);
					getCategoriesData(data, all_data);
					
				}
			});
			function getCategoriesData(array, array2){
			...
			var total_all = 0;
				for(x in array){
						data_list.push(new Array(array[x]["device_name"], parseInt(array[x]["current_device_installs"])));
						total_install += parseInt(array[x]["current_device_installs"]);
				}
				for(x in array2){
						all_data_list.push(new Array(array2[x]["device_name"], parseInt(array2[x]["current_device_installs"]), array2[x]["segment"]));
						total_all += parseInt(array2[x]["current_device_installs"]);
						total_data +=1; 
				}
				total_other = total_all-total_install;	
			...
			function tableDraw(){
				...
				$("table#slime_install tbody tr").html("<td align='center'><h1><b>"+ formatNumber(total_install) +"</b></h1></td>"+"<td align='center'><h1><b>"+ formatNumber(total_data) +"</b></h1></td>");
				for(var i = 0;i<total_data;i++){
				//	console.log(data_list[i][0]);
					$("table#slime_daily_install tbody").append("<tr><td>"+ all_data_list[i][0]+"</td>"+"<td>"+ formatNumber(all_data_list[i][1]) +"</td><td>"+all_data_list[i][2]+"</td></tr>" );
				}
			}
			var init = function(){
